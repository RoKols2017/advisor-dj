...
- Базовые/целевые тесты проходят; актуальный результат проверяется командой `pytest -q tests`.
...
DoD:
- Coverage контролируется порогом CI (`--cov-fail-under=80`); линтеры и типизация настроены.
