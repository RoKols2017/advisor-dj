...
class UserImportForm(forms.Form):
    ...
    file = forms.FileField(label="CSV файл", help_text="CSV файл с колонками: username,fio,department_code")
    ...
    def process_file(self) -> tuple[int, int, list[str]]:
        ...
        if not reader.fieldnames or not required_columns.issubset(set(reader.fieldnames)):
            return 0, 0, ["Неверный формат CSV. Ожидаются колонки: username,fio,department_code"]
