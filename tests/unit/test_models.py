...
class PrintEventTests(TestCase):
    ...
    def setUp(self):
        ...
        self.department = Department.objects.create(name="Test Department", code="TEST")
        self.user = User.objects.create_user(username="testuser", password="testpass", department=self.department)
    ...
    def test_print_event_creation(self):
        ...
        event = PrintEvent.objects.create(
            user=self.user, printer=self.printer, document_id=1, job_id="j1", pages=5, timestamp=timezone.now()
        )
    ...
    def test_get_cost(self):
        ...
        event = PrintEvent.objects.create(
            user=self.user, printer=self.printer, document_id=2, job_id="j2", pages=5, timestamp=timezone.now()
        )
